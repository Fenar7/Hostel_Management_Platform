...
- **Multi-Hostel Admin Occupancy Engine & Interactive Bed Drawer (`/admin/occupancy`, `HostelOccupancyView.tsx`, `hostel-structure/mine/route.ts`):**
  - **API Authorization & Admin Scoping:** Updated `/api/hostel-structure/mine` to support `MAIN_ADMIN` and fallback to the organization's primary hostel if `hostelId` is omitted.
  - **Dedicated Admin Occupancy Workspace:** Created `app/admin/occupancy/page.tsx` and updated `Sidebar.tsx` for `MAIN_ADMIN` to link directly to `/admin/occupancy`.
  - **Multi-Hostel Selector Dropdown:** Integrated an interactive **Hostel Selector Dropdown** (`🏢 Select Hostel: Anywhere Node ▾`) in `HostelOccupancyView.tsx` allowing Main Admins to switch seamlessly between all hostels in their organization.
  - **Interactive Bed Action Drawer:** Added a slide-over drawer when any bed is clicked:
    - **Occupied Bed (`OCCUPIED`):** Renders resident tenant name, avatar, phone, stay start date, next due date, with 1-click `View Stay Details` and `Call Resident` CTAs.
  ...
  - **Admin Stay Details Route Resolution Fix:** Updated `HostelOccupancyView.tsx` to construct `/admin/hostels/${hostelId}/stays/${stayId}` for Main Admins and created a top-level route alias `app/admin/stays/[stayId]/page.tsx` resolving stays across the admin's organization, eliminating 404 errors when viewing resident stay profiles.
  - **Real-time Filters & Tenant Search:** Dynamic filtering by Floor, Sharing Type, Bed Status, and real-time Search across Room Number, Bed Label, or Resident Tenant Name.
