...
- **Raycast & Linear Action Palette Dispatch UI:** Redesigned the onboarding dispatch UI into a compact Raycast Action Sheet (`max-w-md`). Completely eliminated pitch-black code boxes inside light mode dialogs, redundant cards, and browser emojis (`📱`). Integrated Vercel-style token inputs with inline `Copy` buttons right inside the input fields, paired with a clean `Send WhatsApp Invite ↗` CTA.
