...
export default function TicketsPageClient() {
  ...
  const fetchTickets = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/warden/tickets?status=${filter}`);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Data Table */}
      <div className="bg-white dark:bg-[#111111] rounded-[10px] border border-gray-200 dark:border-white/10 overflow-hidden shadow-sm">
