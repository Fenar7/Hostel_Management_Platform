...
describe("Message Templates", () => {
  describe("onboardingLink", () => {
    it("returns correct message without name", () => {
      const msg = onboardingLink("https://nexthome.in/newuser?id=abc");
      expect(msg).toBe(
        "Hello, welcome to NextHome. Please complete your registration here: https://nexthome.in/newuser?id=abc"
      );
    });

    it("returns correct message with name", () => {
      const msg = onboardingLink("https://nexthome.in/newuser?id=abc", "Rahul");
      expect(msg).toBe(
        "Hello Rahul, welcome to NextHome. Please complete your registration here: https://nexthome.in/newuser?id=abc"
      );
    });
  });
  ...
  describe("applicationApprovedPaymentRequest", () => {
    ...
    it("returns detailed breakdown message when breakdown is provided", () => {
      ...
      expect(msg).toContain("NextHome Hostel has been approved");
      ...
      expect(msg).toContain("payment@nexthome");
  ...
  describe("paymentReceiptReady", () => {
    it("returns correct receipt message", () => {
      const msg = paymentReceiptReady("Rahul", 15000, "https://nexthome.in/receipt/123");
      expect(msg).toBe(
        "Hello Rahul, your payment of \u20B9 15,000 has been verified. Download your receipt: https://nexthome.in/receipt/123"
      );
    });
  });
  ...
  describe("rentDueReminder", () => {
    const baseParams = {
      ...
      dueDate: "15 Jul 2026",
      amount: 10000,
      paymentUrl: "https://nexthome.in/tenant",
      daysRemaining: 7,
    };
